...
<section class="py-10 px-2 md:px-10 w-full h-screen bg-white">
    <header class="w-full h-[50px] flex items-center justify-between">
        <h1 class="font1 text-4xl font-semibold text-zinc-800">TEACHER DATA</h1>
        <div class="flex gap-3">
            <div id="open-assign-modal" class="w-auto px-2 h-[50px] group hover:bg-[#28978E] transition-all duration-200 ease-linear rounded-lg flex items-center hover:cursor-pointer">
                <div class="w-full flex items-center justify-center">
                    <p class="font1 text-zinc-500 group-hover:text-zinc-50">Assign Teacher</p>
                </div>
            </div>
            <div id="open-modal" class="w-[150px] h-[50px] bgteal rounded-lg flex px-4 items-center hover:cursor-pointer">
                ...
                <div class="w-[70%] flex items-center justify-center">
                    <p class="font1 text-zinc-200">Add Teacher</p>
                </div>
            </div>
        </div>
    </header>
    ...
    <div class="w-full h-[40px] mt-10 flex items-center justify-end">
        <div class="w-[300px] h-full rounded-[8px] border-2 border-zinc-400 relative font1">
            ...
            <input id="search-data" class="w-full rounded-[20px] px-8 text-zinc-500 h-full outline-none ring-0 focus:outline-none focus:ring-0 focus:border-none" placeholder="Search Teacher...">
        </div>
    </div>
    ...
    <div class="w-full h-[60vh] lg:w-full overflow-hidden overflow-x-scroll overflow-y-scroll mt-3 relative z-10">
        <table class="w-[120vh] lg:w-full text-left text-sm text-white font1 ">
          ...
          <tbody id="data-table-body" class=" min-w-[80vh] text-zinc-900">
            @foreach ($teachers as $teacher)
                ...
                <td class="px-4 py-2 text-zinc-800 font-semibold">{{$teacher->teacher_name}}</td>
                <td class="px-4 py-2"><img src="{{$teacher->profile_picture}}" class="size-12 object-cover rounded-md"></td>
                <td class="px-4 py-2">{{$teacher->school_code}}</td>
                <td class="px-4 py-2">{{$teacher->classroom_count}}</td>
                <td class="px-4 py-2">{{$teacher->assignment_count ?? '0'}}</td>
                {{-- <td class="px-4 py-2">{{$teacher->rank}}</td> --}}
        ...
        {{-- <script type="text/template" id="loader-row-tpl">
            <div class="flex items-center justify-center w-full absolute h-full">
                <x-loader-table/>
            </div>
        </script> --}}
...
<script>
    $(document).ready(function() {
        ...
        function updateUI(data) {
            ...
            if (list.length === 0) {
                html = '<tr><td colspan="7" class="text-center text-zinc-400">No teachers found.</td></tr>';
            } else {
                $.each(list.data, function(index, teacher) {
                    html += '<tr class="border-t text-zinc-500 text-md">';
                    html += '<td class="px-4 py-2 text-zinc-800 font-semibold">' + teacher.teacher_name + '</td>';
                    html += '<td class="px-4 py-2"><img src="' + teacher.profile_picture + '" class="size-12 object-cover rounded-md"></td>';
                    html += '<td class="px-4 py-2">' + teacher.school_code + '</td>';
                    html += '<td class="px-4 py-2">' + (teacher.classroom_count ?? '0') + '</td>';
                    html += '<td class="px-4 py-2">' + (teacher.assignment_count ?? '0') + '</td>';
        ...
        function fetchData(params = {}) {
            $.ajax({
                ...
                beforeSend() {
                    ...
                    // $('#data-table-body').html(loaderRow);
                },
