<div class="h-[600px] w-[70px] left-10 absolute flex items-center justify-center z-50">

    <div class="h-[300px] w-[70px] bg-black/50 round translate-x-2 translate-y-1 absolute"></div>
    <div class="h-[300px] w-[70px] bg-white round flex flex-col items-center justify-between border-[1px] border-black/50 px-1 py-2 absolute z-20">
        ...
        <div class="w-[60px] h-[50px] rounded-[5px] kode flex items-center justify-center relative group hover:cursor-pointer">
            <form method="GET" action="{{ route('games-query') }}">
                ...
                <button>
                    <img class="h-full w-full active:scale-90 animate" src="{{ asset('dashboard/icons/active.png') }}" alt="Toggle Status">
                </button>
            </form>

            <!-- Tooltip for button -->
            <div class="absolute w-auto h-[50px] bg-gray-800 top-[-40px] z-50 left-[-40px] rounded text-center opacity-0 group-hover:opacity-100 animate flex items-center justify-center">
                <p class="text-sm text-white">Toggle Between Active/Inactive</p>
            </div>
        </div>
