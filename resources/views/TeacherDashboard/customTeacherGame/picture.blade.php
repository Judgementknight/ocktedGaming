            ...
            @foreach ($combinedData['Question Data'] as $key => $question)
                @foreach ($question['pictures'] as $picture)
                    <div class="w-full bg-white h-auto pb-4 rounded-2xl overflow-hidden shadow-2xl relative">
                        ...
                        <div class="px-2">
                            <h1 class="font1 text-xl text-zinc-700">{{ $picture['question'] }}</h1>
                            <div class="w-full h-full flex items-center justify-center mt-2">
                                <div class="w-[200px] h-[200px] relative overflow-hidden rounded-lg">
                                    <img src="{{ $picture['image_url'] }}" class="w-full h-full object-cover hover:cursor-pointer" onclick="expandImg(this)">
                                </div>
                            </div>
                            <div class="w-full h-auto mt-2">
                                <label>Your Answer:</label>
                                <input name="answers[{{ $picture['picture_game_id'] }}]" type="text" class="font1 px-2 w-full h-[40px] focus:outline-none border-[1px] border-zinc-400 rounded-lg" placeholder="Type your answer">
                            </div>
                        </div>
                    </div>
                @endforeach
